...
function checkNum(num)
{
   ...
   return number.test(num);
	...
	function submit1(){
		...
		if(form.qq.value.length!=0 && !checkNum(form.qq.value)){
			alert("请输入正确的QQ号", 1500);
			return;
		}
		...
		{
			$.ajax({
				url: BasePath+"/zj/zjminweb/getMemberBytelephone.kq?telephone=" + form.telephone.value + "&openid=" + '${openid?default("")}',
	    		    type: "POST",
	    		    date: "",
	    		    datetype: "json",
	    		    async: false,
	...
	<div class="container info_tx" style="border: 4px solid #FF6699;">
		<div class="body pt_10">
			<ul class="list_ul_card">
				<form id="form1" action="<#if type?? && type=='mod'>${BasePath}/zj/zjminweb/updateMemberInfo.kq<#else>${BasePath}/zj/zjminweb/addMember.kq</#if>" method="post">
					<li data-card=""><header class="center">
						...
						<div class="forms">
							...
							<dl>
								<dt>姓 名：</dt>
								<dd>
									<input type="text" name="name" placeholder="请输入姓名" value="<#if member??>${member.name?default("")}</#if>" maxlength="30" class="input">
								</dd>
							</dl>
							<dl>
								<dt>手 机：</dt>
								<dd>
									<input type="text" name="telephone" placeholder="请输入手机号码" value="<#if member??>${member.telephone?default("")}</#if>" maxlength="30" class="input">
								</dd>
							</dl>
							<!-- 系统字段性别是否必填-->
							<dl>
							...
							<dl>
								<dt>详细地址:</dt>
								<dd>
									<input type="text" name="address" id="Js-address" value="<#if member??>${member.address?default("")}</#if>" placeholder="请输入详细地址" maxlength="100" class="input">
								</dd>
							</dl>
							...
							<div id="defined">
									...
									<dl>
										<dt>QQ：</dt>
										<dd>
											<input type="text" name="qq" placeholder="请填写您的QQ号码" maxlength="80" class="input" value="<#if member??>${member.qq?default("")}</#if>" required="true">
										</dd>
									</dl>								
					...
					<ul class="add_op">
						<li style="padding: 10px 0 0;"><a href="javascript:submit1();" style="width: 100%;"><#if type?? && type=='mod'>修&nbsp;&nbsp;&nbsp;改<#else>提&nbsp;&nbsp;&nbsp;交</#if></a>
						</li>
