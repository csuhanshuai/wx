...
<form id="material_form" method="post" action="updateMultiPTMessage.kq?type=ptMessage">
<input type="hidden" name="id" value="${wxmsg.id}"/>
                <script language="javascript">$(document).ready(wechat_obj.material_multi_init);</script>
    <div class="m_lefter multi">
        <div class="time">${wxmsg.addtime?default('')}</div>
        <div class="first" id="multi_msg_0">
            <div class="info">
                <div class="img" id="MsgImgDetail"><img src="${wxmsg.items[0].picUrl?default('')}"/></div>
                <div class="title">${wxmsg.items[0].title?default('')}</div>
            </div>
            <div class="control">
                <a href="#mod"><img src="${BasePath}/images/icon/mod.gif" /></a>
            </div>
            <input type="hidden" name="Title[]" value="${wxmsg.items[0].title?default('')}" />
            <input type="hidden" name="Url[]" value="${wxmsg.items[0].url?default('')}" />
            <input type="hidden" name="ImgPath[]" value="${wxmsg.items[0].picUrl?default('')}" />
        </div>
            <#list wxmsg.items as it>
            <#if it_index!=0>
            <div class="list" id="multi_msg_${it_index}">
                <div class="info">
    ...
    <div class="m_righter">
        <div class="mod_form">
            ...
            <div class="m_form">
                ...
                <div class="input"><input name="inputTitle" value="${wxmsg.items[0].title?default('')}" type="text" /></div>
