...
<div id="material" class="r_con_wrap" style="min-height:560px;height:auto !important;">
            <form id="material_form" method="post" action="${BasePath}/actInfoManage/updateActInfo.kq?id=<#if actInfo?? ??>${actInfo.id}</#if>">
                ...
                <div class="m_lefter one">
                    <div class="title">图片预览</div>
                      <div class="img" id="MsgImgDetail">
                      	<#if actInfo??>
                      	<a href="${BasePath}/${actInfo.imgDirec}" target="_blank">
							<img src="${BasePath}/${actInfo.imgDirec}">
						</a>
						<#else>
                      	封面图片
                      	</#if>
                ...
                <div class="m_righter">
                    <div class="mod_form">
                        ...
                        <div class="m_form">
                            ...
                            <div class="input"><input name="title" placeholder="活动名称" notnull value="<#if actInfo??>${actInfo.title?default("")}</#if>"  type="text" onKeyUp="javascript:clearError();"/></div>
                            ...
                            <input id="img1" type="hidden" name="ImgPath" value="<#if actInfo?? && actInfo.imgDirec??>${actInfo.imgDirec}</#if>" onChange="javascript:clearError();" >
                            <input id="img2" type="hidden" name="imgDirec" value="<#if actInfo?? && actInfo.imgDirec??>${actInfo.imgDirec}</#if>" >
                            ...
                            <div class="input">
                            	<input id="beginTime" name="beginTime" placeholder="开始时间" value="<#if actInfo??>${actInfo.beginTime?string("yyyy-MM-dd")}</#if>" type="text" onKeyUp="javascript:clearError();" style="width:85px;" onclick="javascript:WdatePicker();"/>
                            	至 <input id="endTime" name="endTime" placeholder="结束时间" value="<#if actInfo??>${actInfo.endTime?string("yyyy-MM-dd")}</#if>" type="text" onKeyUp="javascript:clearError();" style="width:85px;" onclick="javascript:WdatePicker();"/>
                            </div>
                            ...
                            <div><textarea name="content" placeholder="活动描述"><#if actInfo??>${actInfo.content?default("")}</#if></textarea></div>
                        ...
                        <div class="clear"></div>
            <div class="button"><input type="button" onClick="javascript:checkinput();" class="btn_green" name="submit_button" value="提交" />
