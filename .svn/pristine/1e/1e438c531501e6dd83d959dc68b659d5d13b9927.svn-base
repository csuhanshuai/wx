...
<div id="reply_keyword" class="r_con_wrap">
		...
		<div class="tips">分类名称会展示到页面</div></span>
            ...
            <div class="rows" id="img_msg_row">
                ...
                <span class="input">
			<select name='istay'>
			<option value='1' <#if pt??&&pt.istay=="1">selected</#if>>商品分类</option>
			<!--<option value='2' <#if pt??&&pt.istay=="2">selected</#if>>商品标签</option>-->
			</select>
                </span>
                <div class="clear"></div>
            </div>
		...
		<input type="submit" class="btn_green" name="submit_button"  value="提交保存" /><a href="${BasePath}/mallmanage/queryProductType.kq?type=ptypemanage" class="btn_gray">返回</a></span>
                <div class="clear"></div>
            </div>
